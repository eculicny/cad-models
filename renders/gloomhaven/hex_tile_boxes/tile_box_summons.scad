...
summon_token_slot_r = SUMMON_LARGE_DIAMETER / 2 + GENERAL_TOKEN_TOLERANCE;
...
difference() {
    union() {
        difference() {
                            ...
                            translate([HEX_TOKEN_SLOT_APOTHEM, HEX_TOKEN_SLOT_RADIUS, 0]) {
                                for (i = [0:HEX_NUM_ROWS - 1]) {
                                    ...
                                    if (i % 2 == 0) {
                                        ...
                                        translate(
                                            v=[
                                                (base_box_y - (HEX_ROW_MAX_COUNT * 2 * HEX_TOKEN_SLOT_APOTHEM + (HEX_ROW_MAX_COUNT - 2) * GENERAL_WALL_THICKNESS)) / 2 - 3 * GENERAL_WALL_THICKNESS,
                                                i * (3 * HEX_TOKEN_SLOT_RADIUS / 2 + GENERAL_WALL_THICKNESS) - (HEX_TOKEN_SLOT_RADIUS) / 2,
                                            ]
                                        )
                                            square(size=[2 * (HEX_ROW_MAX_COUNT - 1) * (HEX_TOKEN_SLOT_APOTHEM), HEX_TOKEN_SLOT_RADIUS]);
                                    } else {
                                        translate(
                                            v=[
            ...
            // remove walls for 3-hex rubble tile
            color("BLUE")
                translate(
                    v=[
            ...
            // remove walls for 2-hex rubble
            color("BLUE")
                translate(
                    v=[
                        3 * HEX_TOKEN_SLOT_RADIUS / 2,
                        ( (base_box_y - (HEX_ROW_MAX_COUNT - 3) * (2 * HEX_TOKEN_SLOT_APOTHEM + GENERAL_WALL_THICKNESS)) / 2) + HEX_TOKEN_SLOT_APOTHEM + GENERAL_WALL_THICKNESS / 2,
                        base_box_floor,
                    ]
                )
                    cube(size=[HEX_TOKEN_SLOT_RADIUS, HEX_TOKEN_SLOT_APOTHEM + base_box_wall, base_box_z + 5]);
        }
        // replace bottom tips of certain rubble slots
        translate(
            v=[
                // move to insert location in x-direction
                (7 / 2 * HEX_TOKEN_SLOT_RADIUS + base_box_wall) - HEX_TOKEN_SLOT_RADIUS / 2, // could be parameterized off of HEX_NUM_ROWS like 6x box
                // center in y-direction
                (base_box_y - HEX_ROW_MAX_COUNT * 2 * HEX_TOKEN_SLOT_APOTHEM - (HEX_ROW_MAX_COUNT - 1) * base_box_wall) / 2,
                0,
            ]
        ) {
            // based off of previous logic, can probably be simplified
            translate(v=[-HEX_TOKEN_SLOT_RADIUS / 2, 2 * HEX_TOKEN_SLOT_APOTHEM + base_box_wall / 2, 0])
                rotate(a=[0, 0, 90])
                    linear_extrude(height=base_box_z)
                        difference() {
                            regular_polygon(6, HEX_TOKEN_SLOT_RADIUS);
    ...
    // remove walls for element slot
    color("BLUE")
        translate(
            v=[
                6 * HEX_TOKEN_SLOT_RADIUS / 2 + 2 * base_box_wall,
                ( (base_box_y - (HEX_ROW_MAX_COUNT) * (2 * HEX_TOKEN_SLOT_APOTHEM + GENERAL_WALL_THICKNESS)) / 2) + HEX_TOKEN_SLOT_APOTHEM + GENERAL_WALL_THICKNESS / 2,
                base_box_floor,
            ]
        )
